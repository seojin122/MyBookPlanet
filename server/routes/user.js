...
// const { isLoggedIn } = require("../middlewares/index");  // 로그인 확인 미들웨어


router.get("/user", isLoggedIn, getUserProfile);  // 🔹 GET /api/user
...
//   try {
//     const user = await User.findOne({
//       where: { id: req.user.id }, // JWT 또는 세션에서 로그인한 사용자 ID 가져오기
//       include: [
...
// /**  
//  * ✅ 5. 모든 사용자 목록 조회 (로그인한 경우에만 가능)  
//  */
// router.get('/users', isLoggedIn, async (req, res) => {
//   try {
//     const users = await User.findAll();
//     const followingIds = req.user.Followings.map(f => f.id);
...
// /**  
//  * ✅ 6. 특정 사용자 프로필 조회 (닉네임으로 검색)  
//  */
// router.get('/user_profile/:username', async (req, res, next) => {
//   try {
//     const { username } = req.params; // req.params.username → req.params
